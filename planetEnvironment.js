...
export class PlanetEnvironment {
    ...
    adjustCameraForTrajectory() {
            // 1. Sample the projectile's trajectory
                            // 1. Sample the projectile's trajectory points
                const points = [];
                const timeStep = 0.1;
                const maxTime = 10.0; // Adjust this to cover your expected projectile duration
                ...
                for (let t = 0; t <= maxTime; t += timeStep) {
                    ...
                    let y = this.throwStartPosition.y + vy * t - 0.5 * this.gravity * t * t;
                    if (y < 0) { y = 0; break; } // Stop if projectile has hit the ground
                    points.push(new THREE.Vector3(x, y, z));
                }
                if (points.length === 0) return;

                // 2. Compute the bounding box for the trajectory points
                const bbox = new THREE.Box3().setFromPoints(points);
                const center = bbox.getCenter(new THREE.Vector3());
                const size = bbox.getSize(new THREE.Vector3());
                // Use the diagonal half-length as an approximate bounding sphere radius
                const radius = size.length() * 0.5;

                // 3. Calculate the required distance from the center using both vertical and horizontal FOVs
                const fovRad = THREE.MathUtils.degToRad(this.camera.fov);
                // Distance required for vertical FOV:
                const dVert = radius / Math.sin(fovRad / 2);
                // Calculate horizontal FOV based on the camera's aspect ratio:
                const fovH = 2 * Math.atan(Math.tan(fovRad / 2) * this.camera.aspect);
                const dHoriz = radius / Math.sin(fovH / 2);
                const requiredDistance = Math.max(dVert, dHoriz);

                // 4. Get the horizontal (side-on) perpendicular direction relative to the throw direction.
                const horizontalDir = this.initialThrowDirection.clone().setY(0).normalize();
                // Rotate 90° in the horizontal plane to get the perpendicular vector:
                const perp = new THREE.Vector3(-horizontalDir.z, 0, horizontalDir.x).normalize();

                // 5. Position the camera: offset from the center by the required distance along the perpendicular direction.
                const cameraPos = center.clone().add(perp.multiplyScalar(requiredDistance));
                // Optionally adjust the height to better center the arc; tweak the multiplier as needed.
                cameraPos.y += requiredDistance * 0.2;

                // Finally, update the camera position and orientation
